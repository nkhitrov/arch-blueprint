"""Materialize a project as of a git revision, for ``diff --base``.
...
def checkout(project_dir: str, rev: str) -> Iterator[str]:
    ...
    commit = _git(top, "rev-parse", "--verify", "--quiet", f"{rev}^{{commit}}")
    try:
    ...
    archive = _git(top, "archive", "--format=tar", commit.decode().strip(), *paths)
...
def _has_package(root: str, pattern: str) -> bool:
    ...
    return (Path(root) / top).is_dir() or (Path(root) / f"{top}.py").is_file()
